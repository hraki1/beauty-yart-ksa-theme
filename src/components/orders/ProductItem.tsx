...
const ProductItem: React.FC<productItemProps> = ({ item, orderStatus }) => {
  ...
  const t = useTranslations("trackOrders.productItem");
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (error: Error) => {
      ...
      if (error.message.includes("Invalid token")) {
        console.log("Invalid token detected in add to cart, logging out...");
        logout();
        return;
      }
  ...
  return (
    <div className="p-4 border border-gray-200 rounded-lg flex flex-col md:flex-row justify-between md:items-center gap-4 mb-4">
      ...
      <div className="w-full md:w-auto text-end">
        {Array.isArray(item.product?.reviews) && item.product?.reviews.length > 0 ? (
          <StarRating rating={item.product?.reviews[0]?.rating ?? 0} interactive={false} />
        ) : (
          orderStatus && (
            <button
        ...
        `}
            >
              {[1, 2, 3, 4, 5].map((star) => (
                <FaRegStar key={star} size={14} className="text-blue-600transition" />
              ))}
      ...
      <Modal open={isModalOpen} classesName="z-50" onClose={() => setIsModalOpen(false)}>
        <div
          className="bg-white w-full rounded-2xl mx-auto p-0  z-50  relative overflow-hidden backdrop-blur-sm border border-gray-100"
          role="dialog"
          aria-modal="true"
            ...
            <form onSubmit={handleSubmitForm} className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <div className="absolute bottom-3 right-3 bg-white rounded-md px-2 py-1 shadow-sm">
                    <div
                      id="charCount"
                      className={`text-xs font-medium select-none transition-colors ${comment.length > 450 ? 'text-red-500' :
              ...
              <footer className="flex flex-col-reverse sm:flex-row gap-3 pt-4 border-t border-gray-100">
                ...
                <button
                  ...
                  className={`flex-1 px-6 py-3 rounded-xl font-semibold text-white transition-all duration-200 transform hover:scale-[1.02] disabled:scale-100 shadow-lg ${isPending || rating === 0
