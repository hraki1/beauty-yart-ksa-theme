...
const Carousel: React.FC<CarouselProps> = ({ collections }) => {
  ...
  return (
    <div dir="ltr" className="px-3 lg:px-10 relative bg-gradient-to-b from-[#FFEDE4]  to-[#FFF2EC] overflow-visible md:px-8">
      {/* Swiper navigation arrows - hidden on mobile */}
      <button
        className="custom-swiper-prev hidden md:flex absolute left-0 md:left-1 top-1/2 -translate-y-1/2 z-20  hover:bg-[#3740ea] hover:text-white shadow-md rounded-full w-6 md:w-12 h-6 md:h-12 items-center justify-center transition-all duration-200 border border-gray-200"
      ...
      <button
        className="custom-swiper-next hidden md:flex absolute right-0 md:right-1 top-1/2 -translate-y-1/2 z-20 hover:bg-[#3740ea] hover:text-white shadow-md rounded-full w-6 md:w-12 h-6 md:h-12 items-center justify-center transition-all duration-200 border border-gray-200"
      ...
      <div className="rounded relative overflow-visible">
        <Swiper
          ...
          className="h-[459px] md:h-[560px] lg:h-[620px] my-auto rounded-2xl relative overflow-visible"
        >
          <div
          ...
          {displayCollections.map((collection, idx) => {
            ...
            return (
              <SwiperSlide key={collection.collection_id || idx}>
                <div
                  className="relative w-full h-[350px] md:h-[480px] lg:h-[580px] flex flex-row items-center cursor-pointer select-none rounded overflow-visible"
                  onClick={() =>
                    ...
                    <AnimatePresence mode="wait">
                      {activeIndex === idx && (
                        ...
                        >
                          ...
                          <span
                            className={`font-playfair-italic text-lg sm:text-3xl md:text-4xl lg:text-5xl font-semibold text-gray-900 mb-2 sm:mb-3 leading-tight`}
                          >
                            {collection.name}
                          </span>
                          ...
                          <p
                            className="font-playfair-regular text-base sm:text-lg md:text-2xl lg:text-3xl lowercase text-black font-normal tracking-[0.015em] leading-snug my-2 md:my-3"
                          >
                          ...
                          >
                            <button
                              className="text-[12px] mt-8 sm:text-base md:text-xl px-4 sm:px-8 md:px-12 py-1 sm:py-1.5  text-black border-2 cursor-pointer w-fit"
                              onClick={(e) => {
                                e.stopPropagation();
                                router.push(`/shopGrid?collectionId=${collection.collection_id}`);
                              }}
