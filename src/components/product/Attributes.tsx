...
const Attributes = ({
  ...
  const isGloableAvailable = (attributeId: number, optionId: number) => {
    // 1. Get the option text we're checking
    const newOptionText = variantsOptions.find(opt => opt.option_id === optionId)?.option_text || "";

    // 2. Create temporary selection state with the new option
    const tempSelections = {
      ...selectedOptions,
      [attributeId]: newOptionText
    };

    // 3. Get only attributes that are actively selected (non-empty)
    const activeSelections = Object.entries(tempSelections)
      .filter(([value]) => value !== "")
    ...
    // 4. Find variants that:
    //    a) Include ALL currently selected options
    //    b) Don't necessarily include unselected attributes
    return coupledVariants.some(variant => {
      // Check if variant contains all active selections
      const hasAllSelections = Object.entries(activeSelections).every(([attrId, text]) =>
        variant.options.some(opt =>
          opt.attribute_id === Number(attrId) &&
          opt.option_text === text
        )
      );

      // Check if the variant includes the new option we're testing
      const includesNewOption = variant.options.some(opt =>
        opt.attribute_id === attributeId &&
        opt.option_text === newOptionText
      );

      return hasAllSelections && includesNewOption;
    });
  };
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
              {link.attribute.options.map((option) => {
                ...
                const isColorOption = link.attribute.attribute_code
                  .toLowerCase()
                  .includes("color");
                const isAvailable = (isSelected || isAvailableOption(
                  option.attribute_option_id
                ))
                const isGloableAva = (isSelected || isGloableAvailable(
                  option.attribute_id,
                  option.attribute_option_id
                ))
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${!isAvailable
                         ...
                         ${!isGloableAva
                          ? " bg-gray-100 border-gray-200 border-dotted border-2 text-gray-400 opacity-45 relative after:content-[''] after:absolute after:top-1/2 after:left-0 after:w-full after:h-[1px] after:bg-gray-400 after:transform after:rotate-[-15deg] after:origin-center"
                          : "opacity-100 cursor-pointer hover:bg-gray-50 hover:border-gray-300 active:scale-[0.98]"
                        }
