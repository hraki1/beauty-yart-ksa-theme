...
const CartContextProvider: React.FC<CartContextProviderProps> = ({
  ...
  const {
    ...
    onError: (error: Error) => {
      ...
      // Check if error is "Invalid token" and logout user
      if (error.message.includes("Invalid token")) {
        console.log("Invalid token detected in update cart quantity, logging out...");
        logout();
        return;
      }
  ...
  const { mutate: deleteCartItemMutate, isPending: isLoadingDeleteCartItem } =
    useMutation({
      ...
      onError: (error: Error) => {
        ...
        // Check if error is "Invalid token" and logout user
        if (error.message.includes("Invalid token")) {
          console.log("Invalid token detected in delete cart item, logging out...");
          logout();
          return;
        }
  ...
  const { mutate: applyCouponMutate, isPending: isLoadingApplyCoupon } =
    useMutation({
      ...
      onError: (error: Error) => {
        ...
        // Check if error is "Invalid token" and logout user
        if (error.message.includes("Invalid token")) {
          console.log("Invalid token detected in apply coupon, logging out...");
          logout();
          return;
        }
  ...
  const {
    ...
    onError: (error: Error) => {
      console.log("delete applied coupon error:", error);
      
      // Check if error is "Invalid token" and logout user
      if (error.message.includes("Invalid token")) {
        console.log("Invalid token detected in delete applied coupon, logging out...");
        logout();
        return;
      }
  ...
  const {
    ...
    onError: (error: Error) => {
      console.log("save order data error:", error);
      
      // Check if error is "Invalid token" and logout user
      if (error.message.includes("Invalid token")) {
        console.log("Invalid token detected in save order data, logging out...");
        logout();
        return;
      }
