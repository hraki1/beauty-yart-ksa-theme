  ...
  return data.products
    .map((product) => {
      const options = (product.attributes || [])
        .filter(
          (attr) =>
            attr.attribute_id !== undefined &&
            attr.option_id !== undefined &&
            attr.option_text !== undefined
        )
        .map((attr) => ({
          attribute_id: attr.attribute_id as number,
          option_id: attr.option_id as number,
          option_text: attr.option_text as string,
        }));
    ...
    .filter((variant) => variant.url_key && variant.options.length > 0);
}
